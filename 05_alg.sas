...
/*
algorithm 33: Do not perform electrocardiogram on low-risk, asymptomatic patients
denominator: Patients that have no high risk indications, high risk drug prescriptions or symptomatic indications 

Note: Drug/HCC not detemined
*/

%let vars_eleccard = &vars_base lvc 
				   eleccard symptomatic_dx;

%macro alg_eleccard(input);

data output.&input._eleccard_sensitive(keep=&vars_eleccard);
set date.&input;
lvc = eleccard;
run;

data output.&input._eleccard_specific;
set output.&input._eleccard_sensitive;
if not (symptomatic_dx=1);
run;

%patient_level_output(output.&input._eleccard_sensitive, output.&input._eleccard_sensitive_p);
%patient_level_output(output.&input._eleccard_specific, output.&input._eleccard_specific_p);
